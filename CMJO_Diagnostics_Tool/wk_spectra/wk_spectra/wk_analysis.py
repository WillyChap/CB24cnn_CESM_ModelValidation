...
def background_removed_plotter(X,Y,array_in,max_wn_plot,max_freq_plot,figsize,contour_range,\
                               ...
                               matsuno_lines,he,meridional_modes,is_sym,labels,clevels=False):

    fig, ax = plt.subplots(figsize=figsize)
    
    if clevels:
        norm = mpl.colors.BoundaryNorm(clevels, cmap.N)
        cset  = ax.contourf(X,Y,array_in,levels=clevels,cmap=cmap,extend='max',norm =norm)
        cset1 = ax.contour(X,Y,array_in,contour_range_lines,colors='k',linewidths=0.5)
        cb = plt.colorbar(cset,ax=ax,cmap=cmap, norm=norm, extend='max',\
                                       spacing='uniform',ticks=clevels,boundaries=clevels,orientation='vertical')
        tick_font_size = 15
        ...
        cset  = ax.contourf(X,Y,array_in,contour_range,extend='both',cmap=cmap)
        cset1 = ax.contour(X,Y,array_in,contour_range_lines,colors='k',linewidths=0.5)
        plt.colorbar(cset,ax=ax, spacing='uniform')
...
class wk_analysis(object):
    ...
    def plot_background_removed(self,figsize=(10,8),text_size=12,contour_range=(0,2,0.1),min_contour_range_lines=1.1,\
                                matsuno_lines=True,he=[12,25,50],meridional_modes=[1],cmap=nclcmaps.cmap('amwg_blueyellowred'),\
                                max_freq_plot=None,max_wn_plot=None,freq_lines=True,cpd_lines=[3,6,30],labels=False,clevels=False):
        ...
        figA,axA = background_removed_plotter(X,Y,powerplt,self.max_wn_plot,self.max_freq_plot,figsize,\
                                              ...
                                              meridional_modes,False,labels,clevels=clevels)
        ...
        figS,axS = background_removed_plotter(X,Y,powerplt,self.max_wn_plot,self.max_freq_plot,figsize,\
                                              contour_range,contour_range_lines,cmap,text_size,freq_lines,cpd_lines,matsuno_lines,he,\
                                              meridional_modes,True,labels,clevels=clevels)
